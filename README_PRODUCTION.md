...
The project now uses a deploy-focused `requirements.txt`. Do not replace it with a full local `pip freeze`, because desktop and Windows-only packages such as `pywin32`, `PyAudio`, and `PyAutoGUI` will fail on Render's Linux build environment.
...
```bash
gunicorn conquercomputers.wsgi:application
```
...
DJANGO_ALLOWED_HOSTS=.onrender.com,www.conquercomputers.com,conquercomputers.com
```
